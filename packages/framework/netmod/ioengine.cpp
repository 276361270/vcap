...
void IoEngine::setEvent(IConn* pConn, SOCKET s, uint16_t opt, bool bAdd)
{
    ...
    {
        if ( opt & FD_OP_READ && m_readSoSet.size()>0 )
            m_readSoSet.erase(s);
        if ( opt & FD_OP_WRITE && m_writeSoSet.size()>0 )
            m_writeSoSet.erase(s);
    }
    
    if ( opt & FD_OP_CLR )
    {
		if( m_readSoSet.size() > 0 )
			m_readSoSet.erase(s);
		if( m_writeSoSet.size() > 0 )
			m_writeSoSet.erase(s);
...
void IoEngine::run(void*)
{
    ...
    while (!m_nStopping)
    {
		...
		if( m_readSoSet.size() == 0 && m_writeSoSet.size() == 0 )
		{
			::Sleep(20);
			continue;
		}
