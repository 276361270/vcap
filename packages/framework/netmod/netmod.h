...
struct NETMOD_API Packet
{
    ...
    Packet(){ _data = NULL; _bufLen = _dataLen =0; }
    //Packet(char* data, size_t bufLen) : _data(data), _bufLen(bufLen), _dataLen(0){}
    Packet(char* data, size_t bufLen){_data = data; _bufLen = bufLen; _dataLen = 0;} //pre malloc buffer but not insert data
...
* @return int:      SOCKET, INVALID_SOCKET(-1) will be returned if failed
*/
...
* @param pkt    : wrapper of send data
*/
