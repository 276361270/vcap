...
#ifndef __SNOX_BLOCKBUFFER_H_INCLUDE__
#define __SNOX_BLOCKBUFFER_H_INCLUDE__
...
public:
	...
	BlockBuffer()  { m_block = 0; m_size = 0; m_data = NULL; }
	virtual ~BlockBuffer() { free(); }

	inline bool   empty() const	 { return size() == 0; }
	inline size_t block() const	 { return m_block; }
	inline size_t blocksize() const { return allocator::requested_size; }
	inline size_t capacity() const  { return m_block * allocator::requested_size; }
...
public:
    static size_t s_buffer_tick;
    static size_t s_buffer_tick1;
};
...
template <typename BlockAllocator, unsigned MaxBlocks>
size_t BlockBuffer<BlockAllocator, MaxBlocks >::s_buffer_tick = 0;

template <typename BlockAllocator, unsigned MaxBlocks>
size_t BlockBuffer<BlockAllocator, MaxBlocks >::s_buffer_tick1 = 0;
...
{
public:
	ForwardBuffer(Buffer & sb) : m_sb(&sb), m_pos(0) {}
	~ForwardBuffer() { if (m_sb) m_sb->erase(0, m_pos); }
